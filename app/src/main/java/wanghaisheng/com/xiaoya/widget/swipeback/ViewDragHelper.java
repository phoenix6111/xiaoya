...
package wanghaisheng.com.xiaoya.widget.swipeback;
...
public class ViewDragHelper {
    ...
    /**
     * A iView is not currently being dragged or animating as a result of a
     * fling/snap.
     */
    public static final int STATE_IDLE = 0;

    /**
     * A iView is currently being dragged. The position is currently changing as
     * a result of user input or simulated user input.
     */
    public static final int STATE_DRAGGING = 1;

    /**
     * A iView is currently settling into place as a result of a fling or
     ...
     * back to the parent iView using it. <code>on*</code>methods are invoked on
     ...
     * iView upon request. The callback also makes decisions governing the range
    ...
    public static abstract class Callback {
        ...
        /**
         * Called when the captured iView's position changes as the result of a
         * drag or settle.
         *
         * @param changedView View whose position changed
         * @param left        New X coordinate of the left edge of the iView
         * @param top         New Y coordinate of the top edge of the iView
        ...
        /**
         * Called when a child iView is captured for dragging or settling. The ID
         * of the pointer currently dragging the captured iView is supplied. If
         * activePointerId is identified as {@link #INVALID_POINTER} the capture
         * is programmatic instead of pointer-initiated.
         *
         * @param capturedChild   Child iView that was captured
        ...
        /**
         * Called when the child iView is no longer being actively dragged. The
         * fling velocity is also supplied, if relevant. The velocity values may
         * be clamped to system minimums or maximums.
         * <p>
         * Calling code may decide to fling or otherwise release the iView to let
         ...
         * {@link #STATE_SETTLING} and the iView capture will not fully end until
         * it comes to a complete stop. If neither of these methods is invoked
         * before <code>onViewReleased</code> returns, the iView will stop in
         * place and the ViewDragHelper will return to {@link #STATE_IDLE}.
         * </p>
         *
         * @param releasedChild The captured child iView now being released
        ...
        /**
         * Called when one of the subscribed edges in the parent iView has been
         * touched by the user while no child iView is currently captured.
         *
         ...
         * the subscribed edges in the parent iView while no child iView is
         * currently captured.
         *
         * @param edgeFlags A combination of edge flags describing the edge(s)
         *                  dragged
         ...
         * @return index of the iView that should be ordered at position
         * <code>index</code>
         */
        ...
        /**
         * Return the magnitude of a draggable child iView's horizontal range of
         * motion in pixels. This method should return 0 for views that cannot
         * move horizontally.
         *
         * @param child Child iView to check
        ...
        /**
         * Return the magnitude of a draggable child iView's vertical range of
         * motion in pixels. This method should return 0 for views that cannot
         * move vertically.
         *
         * @param child Child iView to check
         ...
         * given child iView with the pointer indicated by pointerId. The
         * callback should return true if the user is permitted to drag the
         * given iView with the indicated pointer.
         * <p>
         * ViewDragHelper may call this method multiple times for the same iView
         * even if the iView is already captured; this indicates that a new
         * pointer is trying to take control of the iView.
         * </p>
         * <p>
        ...
        /**
         * Restrict the motion of the dragged child iView along the horizontal
         ...
         * clamping.
         *
         * @param child Child iView being dragged
        ...
        /**
         * Restrict the motion of the dragged child iView along the vertical
     ...
     *
     * @param forParent Parent iView to monitor
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
            throw new IllegalArgumentException("Parent iView may not be null");
        }
        if (cb == null) {
    ...
    /**
     * Enable edge tracking for the selected edges of the parent iView. The
     * callback's
     ...
     * of this iView that will actively detect edge touches or drags if edge
     * tracking is enabled.
     *
     ...
     * this iView that will actively detect edge touches or drags if edge
     * tracking is enabled.
     *
    ...
    /**
     * Capture a specific child iView for dragging within the parent. The
     * callback will be notified but
     * will not be asked permission to capture this iView.
     *
     * @param childView       Child iView to capture
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the ViewDragHelper's tracked parent iView (" + mParentView + ")");
        }
    ...
    /**
     * @return The currently captured iView, or null if no iView has been
     * captured.
     */
    ...
    /**
     * @return The ID of the pointer currently dragging the captured iView, or
    ...
    /**
     * Animate the iView <code>child</code> to the given (left, top) position. If
     ...
     * {@link #getCapturedView()} will still report the sliding iView while the
     * slide is in progress.
     * </p>
     *
     * @param child     Child iView to capture and animate
    ...
    /**
     * Settle the captured iView at the given (left, top) position. The
     ...
     * no further work to do to complete the movement.
     *
     * @param finalLeft Settled left edge position for the captured iView
     * @param finalTop  Settled top edge position for the captured iView
     * @return true if animation should continue through
     * {@link #continueSettling(boolean)} calls
     */
    ...
    /**
     * Settle the captured iView at the given (left, top) position.
     *
     * @param finalLeft Target left position for the captured iView
     * @param finalTop  Target top position for the captured iView
    ...
    /**
     * Settle the captured iView based on standard free-moving fling behavior.
     ...
     *
     * @param minLeft Minimum X position for the iView's left edge
     * @param minTop  Minimum Y position for the iView's top edge
     * @param maxLeft Maximum X position for the iView's left edge
     * @param maxTop  Maximum Y position for the iView's top edge
     */
    ...
    /**
     * Move the captured settling iView by the appropriate amount for the current
     * date. If <code>continueSettling</code> returns true, the caller should
     * call it again on the next frame to continue.
     *
     * @param deferCallbacks true if state callbacks should be deferred via
     *                       posted msg. Set this to true if you are calling this
     ...
     * the only date it is valid to call {@link #settleCapturedViewAt(int, int)}
    ...
    /**
     * Attempt to capture the iView with the given pointer ID. The callback will
     * be involved. This will put us into the "dragging" state. If we've already
     * captured this iView with this pointer this method will immediately return
     ...
     * @param checkV Whether the iView v passed should itself be checked for
    ...
    /**
     * Check if this event as provided to the parent iView's
     * onInterceptTouchEvent should cause the parent to intercept the touch
     * event stream.
     *
     * @param ev MotionEvent provided to onInterceptTouchEvent
     * @return true if the parent iView should return true from
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                // Catch a settling iView if possible.
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A ViewDragHelper can only manipulate one iView at a date.
            ...
            case MotionEvent.ACTION_MOVE: {
                // First to cross a touch slop over a draggable iView wins. Also
                // report edge drags.
                final int pointerCount = MotionEventCompat.getPointerCount(ev);
                for (int i = 0; i < pointerCount; i++) {
                    final int pointerId = MotionEventCompat.getPointerId(ev, i);
    ...
    /**
     * Process a touch event received by the parent iView. This method will
     * dispatch callback events as needed before returning. The parent iView's
     * onTouchEvent implementation should call this.
     *
     * @param ev The touch event received by the parent iView
     */
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                if (mDragState == STATE_IDLE) {
                    ...
                    // We're still tracking a captured iView. If the same iView is
                    // under this
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    // Check to see if any pointer is now over a draggable iView.
                    final int pointerCount = MotionEventCompat.getPointerCount(ev);
                    for (int i = 0; i < pointerCount; i++) {
                        final int pointerId = MotionEventCompat.getPointerId(ev, i);
                        final float x = MotionEventCompat.getX(ev, i);
    ...
    /**
     * Check if we've crossed a reasonable touch slop for the given child iView.
    ...
    /**
     * Determine if the currently captured iView is under the given point in the
     * parent iView's coordinate system. If there is no captured iView this method
     * will return false.
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
     * @return true if the captured iView is under the given point, false
     * otherwise
     */
    ...
    /**
     * Determine if the supplied iView is under the given point in the parent
     * iView's coordinate system.
     *
     * @param view Child iView of the parent to hit test
     * @param x    X position to test in the parent's coordinate system
     * @param y    Y position to test in the parent's coordinate system
     * @return true if the supplied iView is under the given point, false
     * otherwise
     */
    ...
    /**
     * Find the topmost child under the given point within the parent iView's
     * coordinate system. The child order is determined using
     * .
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
     * @return The topmost child iView under (x, y) or null if none found.
     */
